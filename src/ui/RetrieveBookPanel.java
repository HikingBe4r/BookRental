...
public class RetrieveBookPanel extends JPanel implements ActionListener {
   ...
   Vector<Vector<Object>> removeLists = new Vector<Vector<Object>>();		// 도서 삭제 시 도서리스트
			   ...
			   if(target == registerBookButton) {		// 도서 등록 화면 이동
				  System.out.println("등록 화면 버튼");
				   ...
				   String regex = "\\d{4}\\-\\d{2}\\-\\d{2}";
		            ...
		            if (subjectTF.getText().length() == 0 || writerTF.getText().length() == 0 || publisherTF.getText().length() == 0
		                  || publishDateTF.getText().length() == 0 || genreBox.getSelectedIndex() == -1
		                  || isbnTF.getText().length() == 0) {
				   ...
				   */
				   ...
				   for(int i = 0; i < dm.getRowCount(); i++) {
						...
						if((boolean)dm.getValueAt(i, 0) == true) {			// checkBox check 되어 있으면
							...
							for(int j=0; j<removeList.size(); j++) {
								if(removeList.get(j).get(5).equals("0"))
									removeLists.add(removeList.get(j));		// isbn 동일한 도서 리스트를 저장
							}
						}
					}
				  ...
				  // for(int i=0 ; i<removeLists.size(); i++)
				  //	   idList.add((String)removeLists.get(i).get(0));		// idList에 삭제할 도서 리스트 저장
				   ...
				   for(int i = 0; i < dm2.getRowCount(); i++) {
						...
						if((boolean)dm2.getValueAt(i, 0) == true) {			// checkBox check 되어 있으면
							//isbnList.add((String)dm.getValueAt(i, 1));		// isbn
							//idList = dao.selectBookById((String)dm.getValueAt(i, 5));		// isbn 동일한 도서 리스트를 저장
